...
struct md_rdev {
	...
	/* RHEL 10.2 port fix #7: builtin md core has last_events as
	 * unsigned long (BTF: last_events@32, 8 bytes), not int.  Same
	 * following-field offsets (meta_bdev@40) either way due to alignment,
	 * but match the size to keep the layout byte-exact. */
...
extern int rdev_set_badblocks(struct md_rdev *rdev, sector_t s, sector_t sectors,
			      int is_new);
extern int rdev_clear_badblocks(struct md_rdev *rdev, sector_t s, sector_t sectors,
				int is_new);
...
struct mddev {
	...
	/* RHEL 10.2 port fix #3: 6.12-211 inserted dm_gendisk here (for the
	 * dm-md integration).  Without it, raidkm.ko's struct mddev is 8 bytes
	 * short of the builtin md core's, so every field past ->gendisk
	 * (level/new_level/reshape_position/new_chunk_sectors...) is read at the
	 * wrong offset in run() -> bogus reshape state -> "unsupported reshape
	 * required" abort on a clean create.  This matches the ABI of the
	 * builtin md_mod (confirmed via vmlinux BTF: gendisk@120, dm_gendisk@128,
	 * kobj@136, level@264, reshape_position@352). */
...
#define	UNTIL_STOP	2
	...
	/* RHEL 10.2 port fix #4: 6.12-211 added sync_io_depth here (between
	 * sync_speed_max and parallel_resync).  Confirmed via vmlinux BTF
	 * (sync_io_depth@488).  Without it every field from parallel_resync
	 * onward in the builtin md core is read 4 bytes low by raidkm.ko. */
	int				sync_io_depth;
	...
	/* RHEL 10.2 port fix #5: 6.12-211 added normal_io_events here (after
	 * degraded, before recovery_active).  Confirmed via vmlinux BTF
	 * (normal_io_events@600, unsigned long). */
	unsigned long			normal_io_events;

	/* RHEL 10.2 port fix #6: the builtin md core has recovery_active as a
	 * 32-bit atomic_t @608 (BTF), but keeping the fork's atomic64_t here is
	 * ABI-equivalent and preferred: the 4-byte atomic_t plus the 4 bytes of
	 * padding before recovery_wait (8-byte aligned) occupy the same 8 bytes
	 * an atomic64_t does, so recovery_wait@616 and sizeof(mddev)==2080 are
	 * identical either way.  Keeping atomic64_t also stays consistent with
	 * md.c's atomic64_* uses of this field (md.c isn't built into raidkm.ko,
	 * which never accesses recovery_active directly). */
	atomic64_t			recovery_active; /* sectors scheduled, but not written */
	...
	/* RHEL 10.2 port fix #8: 6.12-211 split out cluster_ops here (after
	 * cluster_info, before good_device_nr).  Confirmed via vmlinux BTF
	 * (cluster_ops@2040).  Without it good_device_nr/noio_flag/deleting/
	 * sync_seq and the trailing bitfields sit 8 bytes low and sizeof(mddev)
	 * is 2072 instead of the builtin's 2080. */
...
{
	/* RHEL 10.2 port fix #1: gendisk->sync_io was removed in 6.12-211.
	 * Mirror upstream: writer is a no-op (reader uses part_stat only). */
}
...
{
	/* RHEL 10 layout: type field prepended, list_head removed,
	 * level moved before name, bitmap_sector added at end.
	 * Verified by binary comparison of stock vs out-of-tree raid456.ko.
	 */
	u32 type;	/* submodule type; 0 = standard personality */
	int level;
	char *name;
	struct module *owner;
	...
	/* RHEL 10 keeps update_reshape_pos here (NULL for raid4/5/6) */
	...
	/* RHEL 10 addition: maps a data sector to its bitmap sector range */
	sector_t (*bitmap_sector)(struct mddev *mddev, sector_t sector);
};
...
/*
 * RHEL 10 / kernel 6.12 renamed register_md_personality →
 * register_md_submodule.  The new API treats personalities, cluster ops
 * and bitmap as variants of a unified submodule; struct md_personality's
 * first fields (type=0, id=level, name, owner) already match the
 * md_submodule_head layout the kernel expects, so we can keep passing
 * &kmec_personality unchanged.
 */
extern int register_md_submodule(struct md_personality *p);
extern int unregister_md_submodule(struct md_personality *p);
...
extern void md_done_sync(struct mddev *mddev, sector_t blocks, int ok);
