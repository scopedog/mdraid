 ...
 *
 * The default max is INT_MAX (no cap): let disk queue depth and the stripe
 * cache HWMARK provide natural backpressure.  Set a lower value explicitly
 * when sharing array bandwidth with latency-sensitive workloads.
 */

static int sysctl_speed_limit_min = 1000;
static int sysctl_speed_limit_max = INT_MAX;
...
layout_store(struct mddev *mddev, const char *buf, size_t len)
{
	...
	if (mddev->pers) {
		...
		mddev->new_layout = n;
		/* Always set layout for new arrays, not just when reshape_position == MaxSector */
		mddev->layout = n;
	}
	mddev_unlock(mddev);
	return err ?: len;
...
sync_speed_show(struct mddev *mddev, char *page)
{
	...
	if (mddev->curr_resync == MD_RESYNC_NONE)
		return sprintf(page, "none\n");
	resync = mddev->curr_mark_cnt - atomic64_read(&mddev->recovery_active);
...
static int status_resync(struct seq_file *seq, struct mddev *mddev)
{
	...
	if (resync < MD_RESYNC_ACTIVE) {
		...
		res = atomic64_read(&mddev->recovery_active);
		/*
	...
	recovery_active = atomic64_read(&mddev->recovery_active);
...
static int is_mddev_idle(struct mddev *mddev, int init)
{
	...
	rdev_for_each_rcu(rdev, mddev) {
		...
		/* RHEL 10.2 port fix #1: gendisk->sync_io removed; drop the term. */
...
void md_done_sync(struct mddev *mddev, sector_t blocks, int ok)
{
	/* another "blocks" (512byte) sectors have been synced */
	atomic64_sub(blocks, &mddev->recovery_active);
	wake_up(&mddev->recovery_wait);
	if (!ok) {
		set_bit(MD_RECOVERY_INTR, &mddev->recovery);
...
void md_do_sync(struct md_thread *thread)
{
	...
	atomic64_set(&mddev->recovery_active, 0);
	last_check = 0;
	...
	while (j < max_sectors) {
		...
		if (!test_bit(MD_RECOVERY_RESHAPE, &mddev->recovery) &&
			...
			wait_event(mddev->recovery_wait,
				   atomic64_read(&mddev->recovery_active) == 0);
			mddev->curr_resync_completed = j;
			if (test_bit(MD_RECOVERY_SYNC, &mddev->recovery) &&
			    j > mddev->recovery_cp)
				mddev->recovery_cp = j;
	...
	repeat:
		if (time_after_eq(jiffies, mark[last_mark] + SYNC_MARK_STEP )) {
			...
			mark[next] = jiffies;
			mark_cnt[next] = io_sectors - atomic64_read(&mddev->recovery_active);
			last_mark = next;
		}
		...
		recovery_done = io_sectors - atomic64_read(&mddev->recovery_active);
		...
		if (currspeed > speed_min(mddev)) {
			...
			if (!is_mddev_idle(mddev, 0)) {
				/*
				 * Partial drain: wait until in-flight rebuild
				 * I/O drops to half its current level before
				 * submitting more.  Steady state keeps ~one
				 * window of rebuild I/O in flight at all times,
				 * letting user I/O compete proportionally
				 * rather than alternating in stop-go bursts.
				 */
				s64 active = atomic64_read(&mddev->recovery_active);
				wait_event(mddev->recovery_wait,
					   atomic64_read(&mddev->recovery_active) <= (active >> 1));
			}
		}
	}
	pr_info("md: %s: %s %s.\n",mdname(mddev), desc,
	...
	wait_event(mddev->recovery_wait, !atomic64_read(&mddev->recovery_active));
...
int rdev_set_badblocks(struct md_rdev *rdev, sector_t s, sector_t sectors,
		       int is_new)
{
	struct mddev *mddev = rdev->mddev;
	int rv;
...
int rdev_clear_badblocks(struct md_rdev *rdev, sector_t s, sector_t sectors,
			 int is_new)
{
	int rv;
	if (is_new)
