...
MODULE_PARM_DESC(default_group_thread_cnt,
		 "Initial group_thread_cnt for new arrays (-1 = auto: min(2, num_online_cpus()/2); 0 = disabled).");
...
static struct r5conf *setup_conf(struct mddev *mddev)
{
	 ...
	 * caps at ~75% of one core. Default to a small positive value scaled by
	 * online CPUs; users can override via the default_group_thread_cnt
	 * module parameter or per-array sysfs (group_thread_cnt).
	 */
	{
		int wcnt = default_group_thread_cnt;
		if (wcnt < 0)
			wcnt = min(2, num_online_cpus() / 2);
