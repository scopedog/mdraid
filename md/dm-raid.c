...
/*
 * raidkm (md level 71, k+m Reed-Solomon) integration.
 *
 * The parity count m and the parity placement (rotating vs dedicated/parity-N)
 * are packed into mddev->layout exactly as md-kmec/km/raid_km.h encodes them.
 * dm-raid persists sb->level/sb->layout verbatim from the mddev, so raidkm
 * geometry round-trips with no new dm_raid_superblock fields.  These defs MUST
 * stay in sync with raid_km.h (RAIDKM_LAYOUT_*).
 */
#define DM_RAIDKM_LEVEL		71
...
#define RAIDKM_MAX_M		8	/* must match RAIDKM_MAX_M in raid_km.h */
...
/* raidkm (level 71): parity count m */
#define __CTR_FLAG_PARITY_COUNT		17 /* 2 */ /* Only with raidkm! */
...
#define CTR_FLAG_PARITY_COUNT		(1 << __CTR_FLAG_PARITY_COUNT)
...
/*
 * "raidkm" (k+m Reed-Solomon, level 71): like raid6 (no "nosync", since m
 * parity blocks must all be written), plus the "parity_count" option that
 * carries m.  No journal options for now.
 */
#define RAIDKM_VALID_FLAGS	(CTR_FLAG_SYNC | \
				 CTR_FLAG_REBUILD | \
				 CTR_FLAG_DAEMON_SLEEP | \
				 CTR_FLAG_MIN_RECOVERY_RATE | \
				 CTR_FLAG_MAX_RECOVERY_RATE | \
...
static struct raid_type {
	...
	/*
	 * raidkm (level 71): parity count m is variable (carried in
	 * mddev->layout, set from the parity_count ctr param), so parity_devs
	 * is a 0 sentinel here.  The 'algorithm' column carries only the
	 * placement bit so get_raid_type_by_ll() can pick the right name from a
	 * persisted layout; "raidkm" = rotating, "raidkm_n" = dedicated parity.
	 */
	{"raidkm",	  "raidkm (k+m RS, rotating parity)",	    0, 3, DM_RAIDKM_LEVEL, RAIDKM_LAYOUT_ROTATING},
	{"raidkm_n",	  "raidkm (k+m RS, dedicated parity)",	    0, 3, DM_RAIDKM_LEVEL, 0}
};
...
static struct arg_name_flag {
	...
	{ CTR_FLAG_PARITY_COUNT, "parity_count" },
};
...
/* Return true, if raid set in @rs is raidkm (level 71) */
static bool rs_is_raidkm(struct raid_set *rs)
{
	return rs->md.level == DM_RAIDKM_LEVEL;
}

/*
 * Number of parity devices for @rs.  For raidkm (level 71) the parity count m
 * is variable and carried in mddev->new_layout (set from the parity_count ctr
 * param); for every other level it is the fixed raid_type constant.
 */
static unsigned int rs_parity_devs(struct raid_set *rs)
{
	if (rs_is_raidkm(rs))
		return raidkm_layout_m(rs->md.new_layout);
	return rs->raid_type->parity_devs;
}
...
{
	/*
	 * raidkm reshapes (data-disk grow, add-parity) run the personality's
	 * COW-staged migration with a CONSTANT data_offset; only disk-adding
	 * reshapes are allowed (see rs_check_reshape / parse_raid_params).
	 */
	return rs_is_raid456(rs) || rs_is_raidkm(rs) ||
...
/* Return true, if raid type in @rt is raidkm (level 71) */
static bool rt_is_raidkm(struct raid_type *rt)
{
	return rt->level == DM_RAIDKM_LEVEL;
}
...
static unsigned long __valid_flags(struct raid_set *rs)
{
	...
	else if (rt_is_raidkm(rs->raid_type))
		return RAIDKM_VALID_FLAGS;
...
static struct raid_type *get_raid_type_by_ll(const int level, const int layout)
{
	...
	while (rtp-- > raid_types) {
		/*
		 * raidkm (level 71): the layout carries m + the rotating bit,
		 * not an ALGORITHM_*.  Match on level and the placement bit
		 * only (the rt 'algorithm' column holds RAIDKM_LAYOUT_ROTATING
		 * for "raidkm" and 0 for "raidkm_n").
		 */
		if (level == DM_RAIDKM_LEVEL) {
			if (rtp->level == DM_RAIDKM_LEVEL &&
			    rtp->algorithm == (layout & RAIDKM_LAYOUT_ROTATING))
				return rtp;
			continue;
		}
...
static int validate_raid_redundancy(struct raid_set *rs)
{
	...
	case 6:
		if (rebuild_cnt > rs->raid_type->parity_devs)
			goto too_many;
		break;
	case DM_RAIDKM_LEVEL:
		/* raidkm tolerates up to m (variable parity count) failures */
		if (rebuild_cnt > rs_parity_devs(rs))
			goto too_many;
		break;
	case 10:
			     ...
			     unsigned int num_raid_params)
{
	...
	for (i = 0; i < num_raid_params; i++) {
		...
		if (!strcasecmp(key, dm_raid_arg_name_by_flag(CTR_FLAG_REBUILD))) {
			...
			/*
			 * raidkm disk-adding reshapes (data-disk grow with
			 * delta_disks > 0, or the add-parity layout step) run
			 * the personality's COW-staged migration, NOT dm-raid's
			 * data_offset-repositioning engine (whose raid456
			 * frontier math corrupts the rotating layout).  Data
			 * never moves: userspace only provides scratch headroom
			 * below a CONSTANT data_offset (see
			 * rs_adjust_data_offsets), and the personality
			 * validates that headroom in its check_reshape.
			 *
			 * SHRINK stays unimplemented in the personality, so
			 * refuse it here at parse time (the only
			 * reliably-reached point).
			 */
			if (rt_is_raidkm(rt) && value < 0) {
				rs->ti->error = "raidkm data-disk shrink (delta_disks < 0) is not supported";
				return -EINVAL;
			}
			...
			raid10_copies = value;
		} else if (!strcasecmp(key, dm_raid_arg_name_by_flag(CTR_FLAG_PARITY_COUNT))) {
			if (test_and_set_bit(__CTR_FLAG_PARITY_COUNT, &rs->ctr_flags)) {
				rs->ti->error = "Only one parity_count argument pair allowed";
				return -EINVAL;
			}

			if (!rt_is_raidkm(rt)) {
				rs->ti->error = "Inappropriate argument: parity_count";
				return -EINVAL;
			}

			if (!__within_range(value, 2, RAIDKM_MAX_M)) {
				rs->ti->error = "Bad value for 'parity_count' (2..8)";
				return -EINVAL;
			}

			/*
			 * Fold the parity count m into mddev->{,new_}layout,
			 * preserving the placement bit seeded from the raid_type
			 * ("raidkm" = rotating, "raidkm_n" = dedicated).
			 */
			rs->md.new_layout = (rs->md.new_layout & RAIDKM_LAYOUT_ROTATING) | value;
	...
	if (rt_is_raidkm(rt)) {
		if (!test_bit(__CTR_FLAG_PARITY_COUNT, &rs->ctr_flags)) {
			rs->ti->error = "raidkm requires a 'parity_count' argument";
			return -EINVAL;
		}
		/* Need at least one data device (k = raid_disks - m >= 1) */
		if (rs->md.raid_disks <= raidkm_layout_m(rs->md.new_layout)) {
			rs->ti->error = "Too few raid devices for the given parity_count";
			return -EINVAL;
		}
		/*
		 * A disk-adding reshape needs the constant data_offset that
		 * hosts the COW scratch zone.  Without the argument, generic
		 * dm-raid would silently skip reshape preparation (no
		 * out-of-place space requested) and the grow request would be
		 * an inert no-op — reject it instead so the contract is
		 * explicit.
		 */
		if (rs->delta_disks > 0 &&
		    !test_bit(__CTR_FLAG_DATA_OFFSET, &rs->ctr_flags)) {
			rs->ti->error = "raidkm reshape requires a 'data_offset' argument (COW scratch headroom)";
			return -EINVAL;
		}
	}
...
static int rs_set_dev_and_array_sectors(struct raid_set *rs, sector_t sectors, bool use_mddev)
{
	...
	else if (rs_is_raidkm(rs))
		/*
		 * raidkm: the COW-staged reshape keeps data_offset constant, so
		 * no out-of-place reshape space is carved out of the data area
		 * (the scratch gap below data_offset is not part of
		 * dev_sectors), and an added disk's role (data vs parity) is
		 * already folded into data_stripes via the parity count
		 * carried in the new layout — do not add delta_disks on top.
		 */
		array_sectors = data_stripes * dev_sectors;
	else
		/* Striped layouts */
		array_sectors = (data_stripes + delta_disks) * (dev_sectors - _get_reshape_sectors(rs));

	/*
	 * raidkm rounds each member's usable size down to a chunk multiple when
	 * the personality starts (raid5_run: dev_sectors &= ~(chunk_sectors-1)).
	 * If the requested per-device length is not already chunk-aligned, the
	 * dm device would be larger than the array actually backs, exposing an
	 * unmaintained tail region beyond resync_max.  Reject such a geometry up
	 * front rather than silently truncating.  (chunk_sectors is a power of 2.)
	 */
	if (rs_is_raidkm(rs) && mddev->new_chunk_sectors &&
	    (dev_sectors & (mddev->new_chunk_sectors - 1))) {
		rs->ti->error = "raidkm target length not a multiple of (data disks x chunk size)";
		return -EINVAL;
	}
...
static void rs_setup_recovery(struct raid_set *rs, sector_t dev_sectors)
{
	...
	if (rs_is_raid0(rs))
		rs->md.recovery_cp = MaxSector;
	/*
	 * A raid6/raidkm set has to be recovered either completely or for the
	 * grown part to ensure proper parity (and Q-syndrome / all m parity).
	 */
	else if (rs_is_raid6(rs) || rs_is_raidkm(rs))
...
/*  Features */
#define	FEATURE_FLAG_SUPPORTS_V190	0x1 /* compat: extended superblock */

/*
 * incompat_features bit: this is a raidkm (level 71) superblock.  Set it so a
 * stock dm-raid (which rejects any non-zero incompat_features in
 * super_validate) refuses the device instead of misreading level 71.
 */
#define	FEATURE_FLAG_RAIDKM		0x1 /* incompat: raidkm k+m metadata */
...
/*
 * True if the requested raidkm layout change is the supported online
 * add-parity step: m -> m+1 at the same parity placement, adding exactly one
 * disk.  Mirrors the personality's raidkm_check_reshape() acceptance, so the
 * dm-side rejection below can give a precise error for everything else.
 */
static bool rs_raidkm_add_parity_requested(struct raid_set *rs)
{
	struct mddev *mddev = &rs->md;

	return raidkm_layout_m(mddev->new_layout) ==
			raidkm_layout_m(mddev->layout) + 1 &&
	       raidkm_layout_is_rotating(mddev->new_layout) ==
			raidkm_layout_is_rotating(mddev->layout) &&
	       rs->delta_disks == 1;
}
...
static int rs_check_reshape(struct raid_set *rs)
{
	...
	if (!mddev->pers || !mddev->pers->check_reshape)
		...
		rs->ti->error = "raid set already reshaping!";
		   ...
		   rs_is_raidkm(rs)))
		rs->ti->error = "Reshaping only supported for raid1/4/5/6/10/raidkm";
	/*
	 * raidkm dm reshape is disk-adding only, via the personality's
	 * COW-staged migration:
	 *  - data-disk grow: new_layout == layout, delta_disks > 0;
	 *  - add-parity: m -> m+1 at the same placement, delta_disks == 1
	 *    (the parity count rides mddev->layout, so this is the one
	 *    legitimate layout change).
	 * Reject everything else:
	 *  - any other layout change (placement flip, m jump > 1, m decrease);
	 *  - chunk-size change: unsupported and unvalidated for raidkm;
	 *  - shrink (delta_disks < 0): raidkm data-disk removal is not implemented
	 *    or validated (it needs an array-size shrink first plus a backup), so
	 *    refuse it rather than drive an untested backward reshape that could
	 *    silently corrupt.
	 */
	else if (rs_is_raidkm(rs) &&
		 mddev->new_chunk_sectors != mddev->chunk_sectors)
		rs->ti->error = "raidkm chunk size change is not supported";
	else if (rs_is_raidkm(rs) && rs->delta_disks < 0)
		rs->ti->error = "raidkm data-disk shrink (delta_disks < 0) is not supported";
	else if (rs_is_raidkm(rs) && mddev->new_layout != mddev->layout &&
		 !rs_raidkm_add_parity_requested(rs))
		rs->ti->error = "raidkm layout change must be a single add-parity step (parity_count m -> m+1, same placement, delta_disks 1)";
	else
		return 0;
...
static void super_sync(struct mddev *mddev, struct md_rdev *rdev)
{
	...
	/* Mark raidkm metadata incompatible so stock dm-raid refuses it. */
	sb->incompat_features = cpu_to_le32(mddev->level == DM_RAIDKM_LEVEL ?
					    FEATURE_FLAG_RAIDKM : 0);
...
static int super_validate(struct raid_set *rs, struct md_rdev *rdev)
{
	...
	/*
	 * The only incompatible feature we understand is FEATURE_FLAG_RAIDKM
	 * (a raidkm k+m superblock); reject anything else.
	 */
	if (le32_to_cpu(sb->incompat_features) & ~FEATURE_FLAG_RAIDKM) {
		rs->ti->error = "Unable to assemble array: Unsupported incompatible feature flags";
		return -EINVAL;
	}
...
static int rs_adjust_data_offsets(struct raid_set *rs)
{
	...
	if (rs_is_raidkm(rs)) {
		/*
		 * raidkm's COW-staged reshape never relocates data: the gap
		 * [0, data_offset) of each data image is a permanent
		 * scratch/journal zone (see raidkm_scratch_io in the
		 * personality), so both offsets are pinned to the constructor
		 * value — no out-of-place toggle, no shift.  Userspace
		 * provides the gap by placing free space at the start of each
		 * image; the personality validates the scratch headroom in
		 * its check_reshape.
		 *
		 * An already-established non-zero offset can never move (the
		 * superblock value is where the data actually lives); reject
		 * a constructor trying to change or drop it.
		 */
		struct md_rdev *r;
		...
		rdev_for_each(r, &rs->md) {
			if (!test_bit(FirstUse, &r->flags) &&
			    r->data_offset &&
			    r->data_offset != rs->data_offset) {
				rs->ti->error = "raidkm data_offset cannot change once set";
				return -EINVAL;
			}
		}
...
static int rs_prepare_reshape(struct raid_set *rs)
{
	...
	} else if (rs_is_raid456(rs) || rs_is_raidkm(rs))
		reshape = true;
...
static int rs_setup_reshape(struct raid_set *rs)
{
	 ...
	 * Not raidkm: its parity count m rides the layout, so the add-parity
	 * reshape legitimately changes layout (m -> m+1) together with
	 * delta_disks == 1 (validated in rs_check_reshape and again by the
	 * personality's check_reshape).
	 */
	if (mddev->delta_disks && !rs_is_raidkm(rs) &&
	    mddev->layout != mddev->new_layout) {
	 ...
	 *
	 * Not raidkm: data_offset never shifts there, so md_finish_reshape()
	 * will not reduce rdev->sectors, and the gap below data_offset is
	 * scratch space, not data — padding rdev->sectors would let the
	 * array address beyond the image's data area.
	 */
	if (!mddev->reshape_backwards && !rs_is_raidkm(rs))
		rdev_for_each(rdev, &rs->md)
			if (!test_bit(Journal, &rdev->flags))
...
static void configure_discard_support(struct raid_set *rs)
{
	...
	/*
	 * XXX: RAID level 4,5,6 and raidkm require zeroing for safety.
	 */
	raid456 = rs_is_raid456(rs) || rs_is_raidkm(rs);
...
static int raid_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if (test_bit(MD_ARRAY_FIRST_USE, &rs->md.flags)) {
		/*
		 * A new raid6/raidkm set has to be recovered to ensure proper
		 * parity (and Q-syndrome / all m parity blocks).
		 */
		if ((rs_is_raid6(rs) || rs_is_raidkm(rs)) &&
		    test_bit(__CTR_FLAG_NOSYNC, &rs->ctr_flags)) {
			ti->error = "'nosync' not allowed for new raid6/raidkm set";
			r = -EINVAL;
			goto bad;
		}
...
size_check:
	 ...
	 * Not raidkm: its conf is owned by the raidkm personality and is not
	 * raid456's r5conf, so raid5_set_cache_size() must not touch it.
	 */
	if (rs_is_raid456(rs)) {
			...
			unsigned int status_flags, char *result, unsigned int maxlen)
{
	struct raid_set *rs = ti->private;
	struct mddev *mddev = &rs->md;
	/* Not raidkm: its mddev->private is not raid456's r5conf. */
	...
	case STATUSTYPE_TABLE:
		...
		/*
		 * Derive the raid type from the persisted level/layout rather than
		 * dereferencing rs->raid_type, mirroring STATUSTYPE_INFO/IMA.  The
		 * raid_type pointer sits immediately after the embedded struct mddev
		 * in struct raid_set and can be left stale/clobbered once the array
		 * is running; the level+layout lookup is authoritative (and, for
		 * raidkm, the name itself — "raidkm" vs "raidkm_n" — is fully
		 * determined by the layout's rotating bit).
		 */
		rt = get_raid_type_by_ll(mddev->new_level, mddev->new_layout);
		if (!rt)
			return;
		...
		DMEMIT("%s %u %u", rt->name, raid_param_cnt, mddev->new_chunk_sectors);
		if (test_bit(__CTR_FLAG_SYNC, &rs->ctr_flags))
		...
		if (test_bit(__CTR_FLAG_PARITY_COUNT, &rs->ctr_flags))
			DMEMIT(" %s %d", dm_raid_arg_name_by_flag(CTR_FLAG_PARITY_COUNT),
